...
const DESTINATIONS = [{
  label: "Nairobi",
  slug: "nairobi",
  image: "/src/assets/destinations/nairobi-destination.jpg",
  desc: "Urban wildlife sanctuary"
}, {
  label: "Samburu",
  slug: "samburu",
  image: "/src/assets/destinations/samburu-destination.jpg",
  desc: "Unique species and cultural encounters"
}, {
  label: "Laikipia",
  slug: "laikipia",
  image: "/src/assets/destinations/laikipia-destination.jpg",
  desc: "Community-owned conservation ranches"
}, {
  label: "Masai Mara",
  slug: "masai-mara",
  image: "/src/assets/destinations/masai-mara-destination.jpg",
  desc: "Great Migration and Big Five experiences"
}, {
  label: "Coastal Kenya",
  slug: "coast",
  image: "/src/assets/destinations/coast-destination.jpg",
  desc: "Coral reefs and marine conservation"
}];
const THEMES = [{
  label: "Wildlife Conservation",
  slug: "wildlife-conservation",
  desc: "Protect endangered species and habitats",
  icon: "🦏"
}, {
  label: "Marine Conservation",
  slug: "marine-conservation",
  desc: "Ocean and coral reef protection programs",
  icon: "🐢"
}, {
  label: "Community Tourism",
  slug: "community-tourism",
  desc: "Support local communities and cultural heritage",
  icon: "🏘️"
}, {
  label: "Conservation Education",
  slug: "conservation-education",
  desc: "Learn through hands-on research experiences",
  icon: "🎓"
}, {
  label: "Sustainable Safari",
  slug: "sustainable-safari",
  desc: "Low-impact wildlife viewing experiences",
  icon: "🦓"
}, {
  label: "Photography Tours",
  slug: "photography-tours",
  desc: "Capture conservation stories through lens",
  icon: "📸"
}, {
  label: "Volunteer Programs",
  slug: "volunteer-programs",
  desc: "Hands-on conservation project participation",
  icon: "🤝"
}, {
  label: "Research Expeditions",
  slug: "research-expeditions",
  desc: "Citizen science and wildlife monitoring",
  icon: "🔬"
}];
export default function HeaderNew() {
  ...
  return <>
      ...
      <header className="w-full bg-background border-b border-border sticky top-0 z-50" style={{
      height: '60px'
    }}>
          ...
          <div className="flex items-center justify-between h-full">
            ...
            <div className="flex items-center flex-shrink-0">
              <Link to="/" className="flex items-center">
                <img src={logoImage} alt="NatuAsili" className="h-8 w-auto object-contain !filter-none !brightness-100 !invert-0" />
              </Link>
            </div>
            ...
            <div className="flex-1 flex justify-center px-4 md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:px-0">
              ...
              <div className="md:hidden">
                <Button variant="ghost" size="sm" onClick={() => setMobileSearchOpen(true)} className="flex items-center gap-2 px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground border border-border rounded-lg" aria-label="Open search">
              ...
              <div className="hidden md:block w-[200px] lg:w-[240px] xl:w-[260px]">
                {desktopSearchOpen ? <AISearchComponent variant="desktop" className="w-full" onClose={() => setDesktopSearchOpen(false)} /> : <Button variant="ghost" size="sm" onClick={() => setDesktopSearchOpen(true)} className="flex items-center gap-2 px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground border border-border rounded-lg w-full justify-start">
                    <Search className="h-4 w-4" />
                    <span>Search...</span>
                  </Button>}
              </div>
            ...
            <div className="flex items-center gap-2 md:gap-4 ml-auto md:ml-0 shrink-0" ref={menuRef}>
              ...
              <div className="hidden lg:block relative">
                <button className="flex items-center gap-1 text-foreground hover:text-primary transition-colors text-sm font-medium py-2" onMouseEnter={() => setOpenMenu("marketplace")} onFocus={() => setOpenMenu("marketplace")}>
                  {t("nav_marketplace")}
              ...
              {!loading && (user && profile ? <div className="hidden lg:block">
                    <AvatarMenu profile={profile} />
                  </div> : <Link to="/auth" className="hidden lg:flex items-center gap-2 px-4 py-2 text-sm border border-border rounded-lg hover:bg-muted transition-colors">
              ...
              <div className="hidden lg:block relative" ref={hamburgerRef}>
                <Button variant="ghost" size="sm" onClick={() => setDesktopHamburgerOpen(!desktopHamburgerOpen)} className="p-2" aria-label="Menu">
              ...
              <Button variant="ghost" size="sm" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2" aria-label="Menu">
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="flex items-center justify-between p-4 border-b border-border">
                  <div className="flex items-center">
                    <img src={logoImage} alt="NatuAsili" className="h-8 w-auto object-contain" />
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => setIsMobileMenuOpen(false)} className="p-2" aria-label="Close menu">
                    <X className="w-5 h-5" />
                  </Button>
                </div>
                ...
                {/* Main navigation */}
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto relative z-[205]" style={{
              pointerEvents: 'auto'
            }}>
                  {/* Marketplace with dropdown */}
                  <div className="space-y-2 relative z-20" style={{
                pointerEvents: 'auto'
              }}>
                    <button className="w-full text-left px-3 py-2 text-sm hover:bg-muted rounded-md flex items-center justify-between relative z-20" style={{
                  pointerEvents: 'auto',
                  touchAction: 'manipulation'
                }} onClick={() => setOpenMenu(openMenu === "mobile-marketplace" ? null : "mobile-marketplace")} aria-expanded={openMenu === "mobile-marketplace"} aria-controls="mobile-marketplace-menu">
                      {t("nav_marketplace")}
                ...
                setIsMobileMenuOpen(false);
                  ...
                  <button className="block w-full text-left px-3 py-2 text-sm hover:bg-muted rounded-md" style={{
                pointerEvents: 'auto',
                touchAction: 'manipulation',
                minHeight: '44px'
              }} onClick={() => {
                ...
                <div className="border-t border-border p-4 space-y-3 mt-auto">
                  {!loading && (user && profile ? <div className="space-y-2">
                        <Link to={profile ? getDashboardPath(profile.role) : "/user-dashboard"} className="block w-full text-center px-4 py-2 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                          Dashboard
                        </Link>
                  ...
                  setIsMobileMenuOpen(false);
                      ...
                      </div> : <Link to="/auth" className="block w-full text-center px-4 py-2 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                        Log in
                      </Link>)}
                </div>
              </div>
      ...
      {mobileSearchOpen && <div className="fixed inset-0 z-[100] bg-background">
          <div className="p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex-1">
                <AISearchComponent variant="mobile" className="w-full" onClose={() => setMobileSearchOpen(false)} />
              </div>
              <Button variant="ghost" size="sm" onClick={() => setMobileSearchOpen(false)} aria-label="Close search" className="md:hidden p-2">
                <X className="w-5 h-5" />
              </Button>
            </div>
          </div>
